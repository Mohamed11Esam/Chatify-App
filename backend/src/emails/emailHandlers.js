...
export const sendWelcomeEmail = async (email, name, clientURL) => {
  ...
  if (sender && isValidEmail(sender.email)) {
    ...
    isValidEmail(process.env.DEFAULT_FROM)
  ) {
    fromValue = process.env.DEFAULT_FROM.trim();
  } else if (
    process.env.FALLBACK_EMAIL &&
    isValidEmail(process.env.FALLBACK_EMAIL)
  ) {
    ...
    console.warn(
      "Using fallback from email. Set a valid sender in ./lib/resend.js or DEFAULT_FROM env var to avoid this."
    );
  }
    ...
    const result = await sendMail({
      ...
      subject:
        process.env.NODE_ENV === "development"
          ? "Welcome to Chatify (development)"
          : "Welcome to Chatify",
      html,
    });
